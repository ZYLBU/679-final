...
fossil_con<-read.csv("D:/679/Fossil energy consumption.csv")
fossil_con<-fossil_con[-c(1,2),]
renewable_con<-read.csv("D:/679/Renewable energy consumption.csv")
...
summary(modelfit_1)
par(mfrow = c(1, 1))
plot(resid(modelfit_1),ylab="Residuals",main="Residuals(Arima(0,1,0)) vs. Time")
...
summary(modelfit_2)
par(mfrow = c(1, 1))
plot(resid(modelfit_2),ylab="Residuals",main="Residuals(Arima(0,1,0)) vs. Time")
...
population<-read.csv("D:/679/Population.csv")
...
production<-read.csv("D:/679/production.csv")
...
ui <- fluidPage(
  titlePanel(
    tags$h1(style = "font-size: 24px;", "Energy trend and prediction")
  ),
  titlePanel(
    tags$h1(
      style = "font-size: 15px;",
      "Question: How can Colorado address the challenge of balancing energy demands for its growing population and economy with the need for conservation and sustainability?"
    )
  ),
  titlePanel(
    tags$h1(
      style = "font-size: 15px;",
      "In the below control panel, you can select the range of year to see the general pattern of energy and input number of years to see prediction results."
    )
  ),
      ...
      sidebarLayout(
        sidebarPanel(
          sliderInput(
            "year_range",
            "Select Year Range:",
            min = min(fossil_con$Date),
            max = max(fossil_con$Date),
            value = c(min(fossil_con$Date), max(fossil_con$Date))
          ),
          numericInput("h_value", "Enter the forecast horizon:", value = 1)
          
        ),
        mainPanel(plotOutput("fossil_plot"),
      ...
      sidebarLayout(
        sidebarPanel(
          sliderInput(
            "year_range2",
            "Select Year Range:",
            min = min(renewable_con$Date),
            max = max(renewable_con$Date),
            value = c(min(renewable_con$Date), max(renewable_con$Date))
            
          ),
          numericInput("h_value2", "Enter the forecast horizon:", value = 1)
          
        ),
        mainPanel(plotOutput("renewable_plot"),
                  plotOutput("forecast_plot2"))
      )
      ...
      sidebarLayout(
        sidebarPanel(
          sliderInput(
            "year_range3",
            "Select Year Range:",
            min = min(production$Date),
            max = max(production$Date),
            value = c(min(production$Date), max(production$Date))
            
          ),
          numericInput("h_value3", "Enter the forecast horizon:", value = 1)
          
        ),
        mainPanel(plotOutput("fossil_plot2"),
                  plotOutput("forecast_plot3"))
      )
    ...
    tabPanel(
      ...
      sidebarLayout(
        sidebarPanel(
          ...
          numericInput("h_value4", "Enter the forecast horizon:", value = 1)
          
        ),
        mainPanel(plotOutput("renewable_plot2"),
                  plotOutput("forecast_plot4"))
      )
...
server <- function(input, output) {
  ...
  output$fossil_plot <- renderPlot({
    ggplot(plot_data(), aes(x = Date, y = Total)) +
      geom_line()
  ...
  output$forecast_plot <- renderPlot({
    plot(
      NULL,
      xlim = c(min(fossil_con$Date), max(fossil_con$Date)),
      ylim = c(0, max(fossil_con$Total)),
      xlab = "Year",
      ylab = "Total"
    )
  })
  ...
  observeEvent(input$h_value, {
    forecast_data <- reactive({
      forecast(modelfit_1, h = input$h_value)
    })
  ...
  plot_data2 <- reactive({
    renewable_con %>%
      filter(Date >= input$year_range2[1] &
               Date <= input$year_range2[2])
  })
  
  output$renewable_plot <- renderPlot({
    ggplot(plot_data2(), aes(x = Date, y = Total)) +
      geom_line()
  })
  ...
  output$forecast_plot2 <- renderPlot({
    plot(
      NULL,
      xlim = c(min(renewable_con$Date), max(renewable_con$Date)),
      ylim = c(0, max(renewable_con$Total)),
      xlab = "Year",
      ylab = "Total"
    )
  })
  ...
  observeEvent(input$h_value2, {
    forecast_data2 <- reactive({
      forecast(modelfit_2, h = input$h_value2)
    })
  ...
  plot_data3 <- reactive({
    production %>%
      filter(Date >= input$year_range3[1] &
               Date <= input$year_range3[2])
  })
  
  output$fossil_plot2 <- renderPlot({
    ggplot(plot_data3(), aes(x = Date, y = fossil.total)) +
      geom_line()
  })
  ...
  output$forecast_plot3 <- renderPlot({
    plot(
      NULL,
      xlim = c(min(production$Date), max(production$Date)),
      ylim = c(0, max(production$fossil.total)),
      xlab = "Year",
      ylab = "Total"
    )
  })
  ...
  observeEvent(input$h_value3, {
    forecast_data3 <- reactive({
      forecast(modelfit_3, h = input$h_value3)
    })
  ...
  plot_data4 <- reactive({
    production %>%
      filter(Date >= input$year_range4[1] &
               Date <= input$year_range4[2])
  })
  
  output$renewable_plot2 <- renderPlot({
    ggplot(plot_data4(), aes(x = Date, y = renewable.total)) +
      geom_line()
  })
  
  
  output$forecast_plot4 <- renderPlot({
    plot(
      NULL,
      xlim = c(min(production$Date), max(production$Date)),
      ylim = c(0, max(production$renewable.total)),
      xlab = "Year",
      ylab = "Total"
    )
  })
  ...
  observeEvent(input$h_value4, {
    forecast_data4 <- reactive({
      forecast(modelfit_4, h = input$h_value4)
    })
